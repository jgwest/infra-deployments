...
configure_compute_workspace() {
  ...
  if [[ -z "$(kubectl get synctargets.workload.kcp.dev ${SYNC_TARGET} --kubeconfig ${KCP_KUBECONFIG} 2>/dev/null)" ]]; then
    ...
    # 
    # Until KCP adds support, StatefulSets should not be used by components, except in the short term and in very limited contexts,
    # and with full understanding of the consequences. - @jonwest
    
    KUBECONFIG=${KCP_KUBECONFIG} kubectl kcp workload sync ${SYNC_TARGET} --syncer-image ghcr.io/kcp-dev/kcp/syncer:v0.9.0 \
      --resources=services,routes.route.openshift.io,statefulsets.apps -o /tmp/${SYNC_TARGET}-syncer.yaml
